...
static int max_name_len(t_manager **file) {
    int temp = 0;
    int max_num = 0;
...
static void column_printer(t_manager **files, int row, int num, int max_len, t_flags* flags) {
    for (int i = 0; i < row; i++) {
...
static void clean_output(t_manager **files, int max_length, int wCol, t_flags* flags) {
    int row = 0;
    int col = 0;
    int count = 0;
    ...
    if (count * max_length > wCol) {
        ...
        column_printer(files, row, count, max_length, flags);
    }
    else {
        for (int i = 0; files[i]; i++) {
            ...
            if (files[i + 1]) {
                tab_printer(mx_strlen(files[i]->name), max_length, flags);
            }
        }
    }
...
void print_clean(t_manager **files, t_flags* flags) {
    ...
    int max_len_of_word = max_name_len(files);

    ioctl(STDOUT_FILENO, TIOCGWINSZ, &window);

    if (isatty(1)) {
        clean_output(files, max_len_of_word, window.ws_col, flags);
    }
    else {
        clean_output(files, max_len_of_word, 80, flags);
    }
}
