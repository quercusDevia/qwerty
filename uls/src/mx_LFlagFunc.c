...
void mx_getMaxVal(t_manager** text, int* blocks, int* max_size, int* max_links, int* max_usname, int* max_grname) {
...
void mx_LFlagFunc(t_manager** text, t_flags* flags) {
    ...
    mx_getMaxVal(text, &numOfBlock, &mSizem, &mOfLink, &longUname, &longGrName);
